...
public class WebSignInProcessSystemTest {
    ...
    @Rollback
    @Sql(scripts = { "/sql/clear_database.sql", "/sql/three_users.sql" })
    void shouldAuthenticateWithAppropriateRoles() throws Exception {
        mockMvc.perform(formLogin("/auth/signin")
                .user("student")
                .password("password"))
    ...
    @Rollback
    @Sql(scripts = { "/sql/clear_database.sql", "/sql/three_users.sql" })
    void shouldRedirectToSignInPageIfSignInAttemptFailed() throws Exception {
        mockMvc.perform(formLogin("/auth/signin")
                .user("unknown")
                .password("invalid"))
                .andExpect(status().is3xxRedirection())
    ...
    void shouldRedirectToSignInPageIfSignOutAttemptSuccessful() throws Exception {
